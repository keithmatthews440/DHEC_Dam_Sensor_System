...
//
//  fona.beginSMS(18645016496);
//  fona.print("Hello World");
//  fona.endSMS();

  delay(5000);
  

 // String Deets = "Give me the raviole";//'{"k":"@b(e0O2d","d":"Hello, World!","t":"Dam_Data","foo":"bar"}';
 // uint8_t lDeets= strlen(Deets);
 // Serial.println(lDeets);
  ...
  if (!fona.TCPconnect("cloudsocket.hologram.io", 9999)) Serial.println(F("Failed to connect to TCP/IP!"));
  if (!fona.MQTTconnect("MQIsdp", MQTT_CLIENT, "735590", "9CwJWKb7GQRBDhssmqUOhWDA6wQHUr")) Serial.println(F("Failed to connect to MQTT broker!"));
  //fona.TCPsend("FOO_BAR",25);  //"k", "@b(e0O2d");
  //delay(1000);
  //fona.MQTTpublish("foo","bar");
  Serial.println(F("Publishing data to their respective topics!"));  

  if (!fona.MQTTpublish("foo", "bar")) Serial.println(F("Failed to publish data!"));
